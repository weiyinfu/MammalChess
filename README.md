...
# 目录结构
* mammal4go：使用golang实现的兽棋AI。
* mammal4py：使用python实现的兽棋AI。
* index.html一个人机对战工具，可以在URL中填入ai=链接这种形式设置AI。


# 扩展AI
只需要提供一个HTTP服务，这个服务接收一个游戏状态字符串，需要设置`Access-Control-Allow-Origin`header来确保可以调用。

传入参数：
* a：一个用逗号隔开的int数组，0~15表示双方棋子，16表示未知，17表示空白
* computerColor：若为0则电脑为0~7，若为1则电脑为8~15
* unknown：未知的棋子列表，也就是尚未出现过的棋子列表
* id：表示当前轮次的id，用来区分请求

返回值：
* strategy：一个数组，如果数组长度为1，则表示翻开`strategy[0]`处的棋子；如果数组长度为2，则表示把`strategy[0]`处的棋子移动到`strategy[1]`处。
* score：表示ai对当前局势的乐观程度，小于0则表示悲观，大于0则表示乐观，这个数值不太重要，可以不用返回
* id：请求的id需要原路返回，用来区分请求。